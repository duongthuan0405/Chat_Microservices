        ...
        <div className="grid md:grid-cols-3 gap-6 p-6">
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-slate-900/30 backdrop-blur-xl rounded-2xl border border-white/10 p-6 h-full flex flex-col">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-6 gap-4 mb-6">
                {friends.slice(0, 12).map((friend) => (
                  <div key={friend.id} className="group cursor-pointer flex flex-col items-center gap-3">
                    <div className="w-full aspect-square relative rounded-2xl overflow-hidden bg-slate-800/50">
                      <img
                        src={friend.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${friend.id}`}
                        alt={friend.name}
                        className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                      />
                      <div className="absolute inset-0 ring-2 ring-transparent group-hover:ring-cyan-500/50 rounded-2xl transition-all" />
                    </div>
